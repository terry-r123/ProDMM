...
python finetunes/finetune_seq2seq/finetune_seq2seq.py \
    ...
    --config_path="ProDMM_seq2seq_reverse_tranlation_ecoli_ckpt/" \
    ...
    --save_interval "20" \
    --pretrain_ckpt "ProDMM_seq2seq_reverse_tranlation_ecoli_ckpt" \
    --frozen_encoder true \
...
python finetunes/finetune_seq2seq/finetune_seq2seq.py \
    --log_gradient_norm="True" \
    --config_path="ProDMM_seq2seq_reverse_tranlation_human_ckpt/" \
    --tokenizer_path="AI4Protein/prolm_1.5B" \
    --pretrain_task="seq2seq" \
    --train_file="$TRAIN_FILE" \
    --valid_file="$VALID_FILE" \
    ...
    --warmup_max_steps="10000" \
    --save_interval "20" \
    --pretrain_ckpt "ProDMM_seq2seq_reverse_tranlation_human_ckpt" \
    --frozen_encoder true \
    --num_training_data 202 \
    --patience 10 \
    --cds_gen true \